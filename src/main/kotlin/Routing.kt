...
fun Application.configureRouting() {
    ...
    routing {
        ...
        put("/tasks/{id}/status/{status}") {
            val taskId: Int = call.parameters["id"]?.toInt() ?:return@put call.respond(HttpStatusCode.NotFound)
            val status: Status = call.parameters["status"]?.let { Status.valueOf(it) } ?:return@put call.respond(HttpStatusCode.BadRequest)
